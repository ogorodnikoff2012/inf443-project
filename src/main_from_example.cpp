...
// Add vcl namespace within the current one - Allows to use function from vcl
// library without explicitly preceding their name with vcl::
using namespace vcl;
...
struct user_interaction_parameters {
  ...
  bool display_frame;   // Boolean indicating wheather the global frame is
                        // displayed
};
...
struct scene_environment {
  camera_around_center camera; // A camera looking at, and rotating around, a
...
// Callback functions
//   Functions called when a corresponding event is received by GLFW (mouse
//   move, keyboard, etc).
void mouse_move_callback(GLFWwindow* window, double xpos, double ypos);
void window_size_callback(GLFWwindow* window, int width, int height);

// Specific functions:
//    Initialize the data of this scene - executed once before the start of the
//    animation loop.
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    if (user.fps_record.event) {
      std::string const title =
          "VCL Display - " + str(user.fps_record.fps) + " fps";
...
void initialize_data() {
  ...
  GLuint const texture_white = opengl_texture_to_gpu(
      image_raw{1, 1, image_color_type::rgba, {255, 255, 255, 255}});
  ...
  // *************************************** //
  vec3 const camera_position        = {2.0f, -3.5f,
                                2.0f}; // position of the camera in space
  vec3 const camera_target_position = {
      0, 0, 0}; // position the camera is looking at / point around which the
                // camera rotates
  ...
  sphere.transform.translate = {0.0f, 0.5f,
                                0.5f}; // translate the position of the sphere
...
void display_scene() {
  ...
  //     Note: scene is used to set the uniform parameters associated to the
  //     camera, light, etc. to the shader

  if (user.display_frame == true)
    draw(frame_visual, scene); // only dislpay the frame when the variable
                               // user.display_frame is true
  draw(sphere, scene);
  draw(plane, scene);

  draw_wireframe(
      plane, scene,
      {0, 0, 1}); // display the triangulation of the quadrangle (in blue)
}
...
void window_size_callback(GLFWwindow*, int width, int height) {
  ...
  scene.projection =
      projection_perspective(50.0f * pi / 180.0f, aspect, 0.1f, 100.0f);
}
